...
class AccountInvoice(models.Model):
            ...
            for invoice in self:
                if invoice.type in ["in_invoice", "in_refund"]:
                    ...
                    for i_line in invoice.invoice_line_ids:
                        ...
                        diff_line = self._anglo_saxon_purchase_move_lines(
                            i_line, res
                        ...
                        for diff in diff_line:
                            ...
                            if add_diff:
                                if (
                                    abs(diff["price_unit"] * diff["quantity"])
                                    > diff_limit
                                ):
                                    raise UserError(
                                        _(
                                            "The price difference for the product %s exceeds the %d limit "
                                        )
                                ...
                                diff[
                                    "quantity"
                                ] = 0.0  # nu mai este necesara inregistrarea cantitatii
    ...
    def trade_discount_distribution(self, res):
        ...
        for (
            ...
            invoice_lines
        ):  ##self.invoice_line_ids.sorted(key=lambda r: r.sequence, reverse=True):
                        ...
                        move.write(
                            {
                                ...
                                "price_unit": (move.value + line_diff_value)
                                / move.product_qty,
                            }
                        )
