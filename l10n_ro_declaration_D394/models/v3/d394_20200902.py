...
class Declaratie394(models.AbstractModel):
    ...
    sistemTVA = fields.Integer(
        string="sistemTVA", xsd_required=True, xsd_type="integer"
    )
    op_efectuate = fields.Integer(
    ...
    schimb_optiune = fields.Integer(
        string="schimb_optiune", xsd_required=True, xsd_type="integer"
    )
    ...
    lista = fields.One2many("D394.30.lista", "lista_Declaratie394_id", string="lista")
    facturi = fields.One2many(
        "D394.30.facturi", "facturi_Declaratie394_id", string="facturi"
    )
    op1 = fields.One2many("D394.30.op1", "op1_Declaratie394_id", string="op1")
    op2 = fields.One2many("D394.30.op2", "op2_Declaratie394_id", string="op2")
    ...
    def generate_facturi(self):
        ...
        invoices = invoices1.filtered(
            lambda r: r.amount_total < 0
            or r.state == "cancel"
            or r.sequence_type in ("autoinv1", "autoinv2")
        )
        for inv in invoices:
            factura = Facturi()
            factura.baza24 = (
                factura.baza20
            ) = factura.baza19 = factura.baza9 = factura.baza5 = 0
            factura.tva24 = (
                factura.tva20
            ) = factura.tva19 = factura.tva9 = factura.tva5 = 0
                    ...
                    for cota in cotas:
                        ...
                        if cota_amount in (5, 9, 19, 20, 24):
                            new_base = inv_curr.with_context(
                                {"date": inv_date}
                            ).compute(line.price_subtotal, comp_currency)
    ...
    def _get_op1(self, invoices):
        def adauga_op1(op1, new):
            if op1:
                try:
                    found = next(
                        index
                        for (index, old) in enumerate(op1)
                        if old.get("tip") == new["tip"]
                        and old.get("tip_partener") == new["tip_partener"]
                        and old.get("cota") == new["cota"]
                        and old.get("cuiP") == new["cuiP"]
                    )
                except:
                    found = None
                        ...
                        if old["op11"]:
                            for new11 in new["op11"]:
                                try:
                                    found11 = next(
                                        index
                                        for (index, old11) in enumerate(old["op11"])
                                        if old11.get("codPR") == new11["codPR"]
                                    )
                                except:
                                    found11 = None
        ...
        for oper_type in oper_types:
            oper_type_inv = invoices.filtered(lambda r: r.operation_type == oper_type)
            partner_types = {invoice.partner_type for invoice in oper_type_inv}
                ...
                for cota in obj_tax.browse(cotas):
                    ...
                    for partner in obj_partner.browse(partners):
                        ...
                        if new_oper_type == "A" and "Ti-ach" in cota.description:
                            new_oper_type = "C"
                        if new_oper_type == "L" and "Ti-livr" in cota.description:
                            new_oper_type = "V"
                        inv_lines = []
                        if partner_type == "2":
                            if new_oper_type == "N":
                                doc_types = [inv.origin_type for inv in part_invoices]
                                for doc_type in doc_types:
                                    ...
                                    for inv_line in inv_lines:
                                        ...
                                        if not fp or (
                                            ("Regim National" in fp.name)
                                            or ("Regim Taxare Inversa" in fp.name)
                                            or (
                                ...
                                for inv_line in inv_lines:
                                    ...
                                    if not fp or (
                                        ("National" in fp.name)
                                        or ("Invers" in fp.name)
                                        or (
                                            ("Scutit" in inv.fiscal_position_id.name)
                                            and inv.partner_type in ("1", "2")
                                        )
                                    ):
                                        tax = inv_line.invoice_line_tax_id
                                        if cota.id in tax.ids:
                            ...
                            if not partner.is_company:
                                if partner.vat:
                                    new_dict["cuiP"] = partner._split_vat(partner.vat)[
                                        1
                                    ]
                                else:
                                    ...
                                    if partner.city_id:
                                        new_dict["locP"] = (
                                            partner.city_id and partner.city_id.name
                                        )
                                    if partner.state_id:
                                        new_dict["judP"] = (
                                            partner.state_id
                                            and partner.state_id.order_code
                                        )
                                    if partner.street:
                                        new_dict["strP"] = (
                                            partner.add_street and partner.add_street
                                        )
                                    if partner.add_number:
                                        new_dict["nrP"] = (
                                            partner.add_number and partner.add_number
                                        )
                                    if partner.add_block:
                                        new_dict["blP"] = (
                                            partner.add_block and partner.add_block
                                        )
                                    if partner.add_flat:
                                        new_dict["apP"] = (
                                            partner.add_flat and partner.add_flat
                                        )
                                    if partner.street2:
                                        new_dict["detP"] = (
                            ...
                            for inv_line in inv_lines:
                                ...
                                if not fp or (
                                    ...
                                    or ("Invers" in fp.name)
                                    or (
                                        ("Scutit" in inv.fiscal_position.name)
                                        and inv.partner_type == "2"
                                    )
                                ):
                                    tax = inv_line.invoice_line_tax_id
                                    if cota.id in tax.ids:
                                    ...
                                    "Scutit" not in inv.fiscal_position.name
                                ):
                                    inv_type = inv_line.invoice_id.type
                                    if inv_type in ("out_invoice", "out_refund"):
                                        tax = inv_line.product_id.taxes_id
                            ...
                            for line in inv_lines:
                                ...
                                if new_oper_type in ("L", "A", "AI"):
                                    taxes += inv_curr.with_context(
                                        {"date": inv_date}
                                    ).compute(line.price_taxes, comp_curr)
                                if (new_oper_type == "C") or (
                                    (new_oper_type == "L")
                                    and (line.invoice_id.partner_type in ("3", "4"))
                                ):
                                    taxes += inv_curr.with_context(
                                        {"date": inv_date}
                                    ).compute(
                            ...
                            new_dict = {
                                ...
                                "cota": cota_amount,
                                "cuiP": partner.vat
                                and partner._split_vat(partner.vat)[1]
                                or "-",
                                "denP": partner.name.replace("&", "-").replace('"', ""),
                        ...
                        if inv_lines:
                            ...
                            op11 = []
                            if (
                                partner_type == "1" and new_oper_type in ("V", "C")
                                    ...
                                    if partner_type == "1":
                                        cod_lines = [
                                            ...
                                            for line in inv_lines.filtered(
                                                lambda r: r.product_id.d394_id.id
                                                == code.id
    ...
    def build_file(self):
        ...
        if self.user.partner_id.comercial_partener_id:
            ...
            xmldict.update(
                {
                    "functie_intocmit ": self.user.partner_id.vat
                    and self.user.partner_id.vat[2:]
                    or ""
                }
            )

        xmldict.update({"optiune": 1, "schimb_optiune": 1, "prsAfiliat": 1})
        obj_invoice = self.env["account.move"]
...
class Facturi(models.AbstractModel):
    ...
    tip_factura = fields.Char(
        string="tip_factura", xsd_required=True, xsd_type="string"
    )
...
class Op1(models.AbstractModel):
    ...
    tip_partener = fields.Integer(
        "D394.30.int_tippartenerop1stype",
        string="tip_partener",
        xsd_required=True,
        xsd_type="integer",
    )
    ...
    tip_document = fields.Char(
        xsd_required=True, xsd_type="string", string="tip_document"
    )
...
class Op2(models.AbstractModel):
    ...
    total = fields.Many2one("D394.30.intpoz15stype", string="total", xsd_required=True)
    baza20 = fields.Many2one(
        "D394.30.intpoz15stype", string="baza20", xsd_required=True
    )
    baza9 = fields.Many2one("D394.30.intpoz15stype", string="baza9", xsd_required=True)
    baza5 = fields.Many2one("D394.30.intpoz15stype", string="baza5", xsd_required=True)
    TVA20 = fields.Many2one("D394.30.intpoz15stype", string="TVA20", xsd_required=True)
    TVA9 = fields.Many2one("D394.30.intpoz15stype", string="TVA9", xsd_required=True)
    TVA5 = fields.Many2one("D394.30.intpoz15stype", string="TVA5", xsd_required=True)
    baza19 = fields.Many2one(
        "D394.30.intpoz15stype", string="baza19", xsd_required=True
    )
    TVA19 = fields.Many2one("D394.30.intpoz15stype", string="TVA19", xsd_required=True)
...
class Rezumat1(models.AbstractModel):
    ...
    cota = fields.Many2one("D394.30.int_cotetvastype", string="cota", xsd_required=True)
...
class Rezumat2(models.AbstractModel):
    ...
    bazaFSL = fields.Integer(string="bazaFSL", xsd_required=True, xsd_type="integer")
    TVAFSL = fields.Integer(string="TVAFSL", xsd_required=True, xsd_type="integer")
    bazaFSA = fields.Integer(string="bazaFSA", xsd_required=True, xsd_type="integer")
    TVAFSA = fields.Integer(string="TVAFSA", xsd_required=True, xsd_type="integer")
    bazaFSAI = fields.Integer(string="bazaFSAI", xsd_required=True, xsd_type="integer")
    TVAFSAI = fields.Integer(string="TVAFSAI", xsd_required=True, xsd_type="integer")
    bazaBFAI = fields.Integer(string="bazaBFAI", xsd_required=True, xsd_type="integer")
    TVABFAI = fields.Integer(string="TVABFAI", xsd_required=True, xsd_type="integer")
    ...
    baza_incasari_i1 = fields.Integer(string="baza_incasari_i1", xsd_type="integer")
    tva_incasari_i1 = fields.Integer(string="tva_incasari_i1", xsd_type="integer")
    baza_incasari_i2 = fields.Integer(string="baza_incasari_i2", xsd_type="integer")
    tva_incasari_i2 = fields.Integer(string="tva_incasari_i2", xsd_type="integer")
    bazaL_PF = fields.Integer(string="bazaL_PF", xsd_required=True, xsd_type="integer")
    tvaL_PF = fields.Integer(string="tvaL_PF", xsd_required=True, xsd_type="integer")
...
class SerieFacturi(models.AbstractModel):
    ...
    tip = fields.Many2one("D394.30.intint1_4stype", string="tip", xsd_required=True)
