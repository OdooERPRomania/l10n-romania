...
class ANAFD394(models.TransientModel):
    ...
    def _get_default_declaration(self):
        d394 = self.env["anaf.declaration"].search([("name", "=", "d394")], limit=1)
        if d394:
            return d394
        return super(ANAFD394, self)._get_default_declaration()

    declaration_id = fields.Many2one(default=_get_default_declaration)
    optiune = fields.Boolean("ANAF Crosschecking", related="company_id.optiune")
    schimb_optiune = fields.Boolean("Allow ANAF Crosschecking")
    prsAfiliat = fields.Boolean("Operations with affiliated companeis")
